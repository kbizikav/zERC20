...
import { NUM_BATCH_INVOICES, GLOBAL_TRANSFER_TREE_HEIGHT } from '../core/constants.js';
...
export function formatFieldElement(value: string, label: string): string {
  ...
  try {
    ...
    throw new Error(
      `${label} must fit within 32 bytes: ${normalized} (${error instanceof Error ? error.message : String(error)})`,
    );
  }
}
