import { createSingleWithdrawWasm, createWithdrawNovaWasm, } from '../wasm/index.js';
import { normalizeHex } from '../utils/hex.js';
import { appendDummySteps, formatFieldElement, hexToBytes, toFieldHex, toLeafIndexString, } from './proofUtils.js';
export async function computeSingleTeleportProof(params) {
    const wasm = await createSingleWithdrawWasm(params.wasmArtifacts.localPk, params.wasmArtifacts.localVk, params.wasmArtifacts.globalPk, params.wasmArtifacts.globalVk);
...
function sortProofsByLeafIndex(events, proofs) {
...
export async function computeNovaProof(params) {
    ...
    const wasm = await createWithdrawNovaWasm(params.wasmArtifacts.localPp, params.wasmArtifacts.localVp, params.wasmArtifacts.globalPp, params.wasmArtifacts.globalVp);
    const z0 = [
    ...
    try {
        ...
        console.error('[zkERC20] computeNovaProof failed', { firstSteps: debugSiblings.slice(0, 2), totalSteps: steps.length }, error);
        throw new Error(error instanceof Error
