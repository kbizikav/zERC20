#!/usr/bin/env python3
"""Call setAllowedProver on every Verifier proxy listed in a tokens.json configuration."""
...
def parse_args() -> tuple[Path, str, list[str]]:
    parser = argparse.ArgumentParser(
        usage="run_set_allowed_prover.py --prover ADDRESS [--file PATH] [--] [cast flags...]",
        description=(
            "Reads a tokens.json-formatted file and calls setAllowedProver(prover) "
            "on every Verifier proxy."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  ./run_set_allowed_prover.py --prover 0xABCD...\n"
            "  ./run_set_allowed_prover.py --prover 0xABCD... --file ../config/deployed/testnet/tokens.zusdc.testnet.json\n"
            "  ./run_set_allowed_prover.py --prover 0xABCD... --chain sepolia\n"
            "  ./run_set_allowed_prover.py --prover 0xABCD... -- --ledger\n"
        ),
    )
    parser.add_argument("--prover", required=True, help="Address to grant prover permission")
    parser.add_argument("--file", dest="tokens_file", help="Path to tokens.json (defaults to ../config/tokens.json)")
    parser.add_argument("--chain", dest="chain_filter", help="Only target the verifier on this chain (matched against label)")
    args, cast_args = parser.parse_known_args()
    ...
    if cast_args and cast_args[0] == "--":
        cast_args = cast_args[1:]

    if args.chain_filter:
        os.environ["_SET_PROVER_CHAIN_FILTER"] = args.chain_filter

    return tokens_path, args.prover, cast_args
...
def has_signer_flag(args: Sequence[str]) -> bool:
    for arg in args:
        flag = arg.split("=", 1)[0]
...
def ensure_private_key(extra_args: Sequence[str]) -> None:
    if has_signer_flag(extra_args):
        return
    if not os.environ.get("PRIVATE_KEY"):
        raise ConfigError("PRIVATE_KEY environment variable must be set (or pass a signer flag like --ledger)")
...
def parse_config(tokens_path: Path) -> list[TokenConfig]:
    ...
    chain_filter = os.environ.get("_SET_PROVER_CHAIN_FILTER")
...
def run_cast_send(
    ...
    cmd = [
        "cast", "send",
        verifier,
        "setAllowedProver(address)",
        prover,
        "--rpc-url", rpc_url,
    ]
    if legacy_tx:
        cmd.append("--legacy")
    if not has_signer_flag(extra_args):
...
def main() -> None:
    tokens_path, prover, cast_args = parse_args()
    if not tokens_path.is_file():
        raise ConfigError(f"tokens file not found at {tokens_path}")

    ensure_command_available("cast")
    ensure_private_key(cast_args)

    tokens = parse_config(tokens_path)

    print(f"Setting allowed prover {prover} on {len(tokens)} Verifier(s)")
    print()

    for token in tokens:
        suffix = " (legacy tx)" if token.legacy_tx else ""
        print(f"[{token.label}] setAllowedProver on {token.verifier_address}{suffix}")

        run_cast_send(
            verifier=token.verifier_address,
            prover=prover,
            rpc_url=token.rpc_url,
            legacy_tx=token.legacy_tx,
            extra_args=cast_args,
        )
        print(f"[{token.label}] Done")
        print()

    print(f"All {len(tokens)} setAllowedProver call(s) completed")
...
if __name__ == "__main__":
    try:
        ...
        print(f"cast command failed with exit code {exc.returncode}: {cmd}", file=sys.stderr)
