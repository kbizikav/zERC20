...
fn build_decider_circuit_for_root() -> anyhow::Result<DeciderEthCircuit<G1, G2>> {
    ...
    let (cf_cs_pp, _) =
        Pedersen::<G2>::setup(&mut rng, pedersen_len).map_err(|e| anyhow::anyhow!(e.to_string()))?;
...
fn benchmark_decider_root_new_from_accumulator() {
    ...
    println!("[3] Transcript::new_from_accumulator time: {:?}", transcript_time);

    println!("\n=== Summary ===");
    println!("Total time: {:?}", load_time + circuit_time + transcript_time);
    println!("  [1] PTAU load:              {:?} ({:.1}%)", load_time, 100.0 * load_time.as_secs_f64() / (load_time + circuit_time + transcript_time).as_secs_f64());
    println!("  [2] Circuit build:          {:?} ({:.1}%)", circuit_time, 100.0 * circuit_time.as_secs_f64() / (load_time + circuit_time + transcript_time).as_secs_f64());
    println!("  [3] new_from_accumulator:   {:?} ({:.1}%)", transcript_time, 100.0 * transcript_time.as_secs_f64() / (load_time + circuit_time + transcript_time).as_secs_f64());
}
