...
3. Choose one of the available redeem paths:
   ...
   - **Gasless Redeem** — the relay node submits the redeem transaction on your behalf
4. If you use the gasless path, review the quoted relay fee and approve the confirmation dialog
...
### When to use Gasless Redeem

Use **Gasless Redeem** when the destination chain wallet does not already have enough native gas to
submit the redeem transaction directly.

Notes:

- The relay fee is deducted from the redeemed zERC20 amount
- The frontend asks for confirmation before signing the fee authorization
- The gasless path still prepares the same proof resources; it only changes who submits the final
...
- **Relayer fee estimate failed** — The relay node could not provide a fee quote. Retry, or switch off **Gasless Redeem** and redeem directly if you already have native gas.
